...
${metadata.isbn ? `ISBN: ${metadata.isbn}` : ''}`;
  ...
  // Save the PDF
  doc.save(
    `${metadata.title.replace(/[^a-z0-9]/gi, '_').toLowerCase()}_print_ready.pdf`
  );
}

// Export to InDesign-compatible format (IDML)
export function exportToIDML(_chapters: Chapter[], _metadata: BookMetadata) {
...
export function exportToPrintReadyEPUB(
  _chapters: Chapter[],
