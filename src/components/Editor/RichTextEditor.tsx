...
export default function RichTextEditor({ initialContent, onChange, chapterId }: RichTextEditorProps) {
  ...
  const editor = useEditor({
    extensions: [
      ...
      Placeholder.configure({
        placeholder: 'Start writing your story... (Press Ctrl+Space for AI suggestions)',
      }),
    ],
  ...
  const analyzeContext = useCallback(() => {
    ...
    const isSceneDescription = /\b(was|were|is|are|stood|sat|walked|moved|looked|felt)\b/i.test(currentText)
    
    // Detect if we're in action
    const isAction = /\b(ran|jumped|fought|attacked|defended|moved|grabbed|threw)\b/i.test(currentText)
  ...
  const handleAutoComplete = useCallback(async (completionType: 'sentence' | 'paragraph' | 'scene' | 'dialogue' = 'sentence') => {
    ...
    try {
      const response = await fetch('/api/ai/autocomplete', {
        ...
        body: JSON.stringify({
          ...
          maxWords: detectedType === 'sentence' ? 25 : 
                   detectedType === 'paragraph' ? 100 : 
                   detectedType === 'scene' ? 200 : 150
        })
      })
  ...
  const generateSuggestions = useCallback(async () => {
    ...
    try {
      ...
      const suggestionTypes: Array<'sentence' | 'paragraph' | 'scene' | 'dialogue'> = []
    ...
    <div className="min-h-[500px] border border-border rounded-lg bg-background">
      <div className="border-b border-border p-2 flex flex-wrap gap-2">
        ...
        <div className="border-l border-border ml-2 pl-2 flex gap-2">
          ...
          >
            {isAutoCompleting ? <FaSpinner className="animate-spin" /> : <FaMagic />}
            Complete Sentence
          </button>
          <button
          ...
          >
            {isAutoCompleting ? <FaSpinner className="animate-spin" /> : <FaLightbulb />}
            Get Suggestions
          </button>
          <button
      ...
      <div className="p-4 relative">
        ...
        {showSuggestions && suggestions.length > 0 && (
          <div className="absolute top-4 right-4 w-80 bg-white border border-border rounded-lg shadow-lg p-4 z-10">
            <h3 className="font-medium text-foreground mb-3">Writing Suggestions</h3>
          ...
          <div className="absolute top-4 right-4 w-80 bg-white border border-border rounded-lg shadow-lg p-4 z-10">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <kbd className="px-2 py-1 bg-secondary/10 rounded text-xs">Ctrl+Enter</kbd>
              </div>
              <div className="flex justify-between">
                <span>Complete Paragraph:</span>
                <kbd className="px-2 py-1 bg-secondary/10 rounded text-xs">Ctrl+Shift+Enter</kbd>
              </div>
              <div className="flex justify-between">
                <span>Get Suggestions:</span>
                <kbd className="px-2 py-1 bg-secondary/10 rounded text-xs">Ctrl+Space</kbd>
              </div>
              <div className="flex justify-between">
                <span>Close Panels:</span>
                <kbd className="px-2 py-1 bg-secondary/10 rounded text-xs">Escape</kbd>
              </div>
            </div>
            <button
