...
interface BookActions {
  ...
  autoCompleteText: (params: {
    text: string;
    context: string;
  }) => Promise<string>;
  rewriteChapter: (params: {
    chapterId: string;
    instructions: string;
    style?: string;
    tone?: string;
  ...
  generateDialogue: (params: {
    characterId: string;
    context: string;
    style?: 'casual' | 'formal' | 'humorous';
  }) => Promise<string>;
  improveWriting: (params: {
    text: string;
    improve: 'clarity' | 'vividness' | 'conciseness';
  }) => Promise<string>;
  generateChapterTitle: (params: {
    content: string;
    style?: 'literal' | 'metaphorical' | 'dramatic';
  }) => Promise<string[]>;

  // Save generated content
  saveGeneratedCharacters: (characters: Character[]) => void;
  saveGeneratedPlot: (plot: string) => void;
  ...
  persist(
    (set, get) => ({
      ...
      metadata: {
        ...
        status: 'in-progress',
        synopsis:
          'A young adventurer discovers an ancient city hidden in the mountains, but unlocking its secrets comes with a terrible price.',
        themes: ['Adventure', 'Discovery', 'Sacrifice'],
        setting:
          'A mystical mountain range in a world where magic and technology coexist.',
        ...
        : [
            {
              ...
              role: 'protagonist',
              description:
                "A determined young adventurer in her early twenties, Sarah is intelligent, curious, and willing to take risks to uncover the truth about her family's past.",
              background:
                'Raised by her grandfather after her parents disappeared when she was young. She has spent years studying ancient texts and maps, preparing for this journey.',
              motivations: [
                'Find her missing parents',
                'Uncover family secrets',
                "Prove herself worthy of her grandfather's legacy",
              ],
              relationships: [],
            },
            {
              ...
              role: 'supporting',
              description:
                "Sarah's grandfather, a renowned scholar and explorer who discovered Eldara decades ago but never returned from his last expedition.",
              background:
                'Former professor of archaeology who specialized in ancient civilizations. He was the first to map the location of Eldara and understand its significance.',
              motivations: [
            ...
            },
            {
              ...
              role: 'supporting',
              description:
                "An ancient being of living stone who has protected Eldara for centuries, maintaining the balance between the city's power and the outside world.",
              background:
                'Created by the original builders of Eldara to serve as both protector and guide. Has witnessed the rise and fall of many civilizations.',
              motivations: [
                "Protect Eldara's secrets",
                'Test worthy visitors',
                'Maintain ancient balance',
              ],
              relationships: [],
            },
          ],
      ...
      plot: {
        summary:
          "A young adventurer named Sarah Chen follows her grandfather's map to discover the lost city of Eldara, only to find that unlocking its ancient secrets comes with a terrible price that could change the world forever.",
        outline: [
      ...
      updateMetadata: metadata =>
        set(state => ({
          ...
          book: { ...(state.book ?? state.metadata), ...metadata },
        })),
      ...
      updateChapter: (id, chapter) =>
        set(state => ({
          chapters: state.chapters.map(c =>
            c.id === id
              ? { ...c, ...chapter, lastModified: new Date().toISOString() }
              : c
          ),
        })),
      ...
      updateSetting: setting =>
        set(state => ({ setting: { ...state.setting, ...setting } })),
      ...
      generateChapter: async ({
        ...
        try {
          const newChapter = await apiClient.generateChapter({
            title,
            prompt,
            style,
            tone,
      ...
      generateCharacterIdeas: async ({
        ...
        try {
          return await apiClient.generateCharacters({
            genre,
            count,
            role,
            archetype,
      ...
      generatePlotIdeas: async ({
        ...
        try {
          const result = await apiClient.generatePlot({
            genre,
            theme,
            complexity,
            characterRelationships,
      ...
      generateDialogue: async ({ characterId, context, style }) => {
        void style;
      ...
      generateChapterTitle: async ({ content, style }) => {
        void style;
        try {
          ...
          set(state => ({
            ...
            chapters:
              story.chapters?.map((chapter: any, index: number) => ({
                ...
                content: chapter.content,
                summary:
                  chapter.summary || `Chapter ${index + 1}: ${chapter.title}`,
      ...
      checkAchievements: () => {
        ...
        const lastSession =
          state.writingSessions[state.writingSessions.length - 1];
        const currentStreak =
          lastSession && lastSession.date === today
            ? Math.min(state.userProgress.currentStreak + 1, 100)
            : 0;
        ...
        ACHIEVEMENT_BADGES.forEach(badge => {
          ...
          let progress = 0;
          const maxProgress = badge.unlockCondition.value;
          ...
          switch (badge.unlockCondition.type) {
            ...
            case 'sessions':
              ...
              isUnlocked =
                state.writingSessions.length >= badge.unlockCondition.value;
              break;
            case 'sessionWords':
              const maxSessionWords = Math.max(
                ...state.writingSessions.map(s => s.wordCount),
                0
              );
        ...
        set(state => ({
          achievements: [
            ...state.achievements.filter(
              a => !newAchievements.find(n => n.id === a.id)
            ),
            ...newAchievements,
          ],
        ...
        set(state => ({
          ...
          userProgress: {
            ...
            achievementsUnlocked:
              state.achievements.filter(a => a.isUnlocked).length + 1,
            totalXP:
              state.userProgress.totalXP +
              (state.achievements.find(a => a.id === achievementId)?.xpReward ||
                0),
          },
        }));
      },
      ...
      getDailyChallenge: () => {
        ...
        if (
          ...
          new Date(state.dailyChallenge.expiresAt) > new Date()
        ) {
      ...
      completeDailyChallenge: (challengeId: string) => {
        const state = get();
        if (
          state.dailyChallenge?.id === challengeId &&
          !state.dailyChallenge.isCompleted
        ) {
          set(state => ({
            dailyChallenge: {
              ...state.dailyChallenge!,
      ...
      addWritingSession: (wordCount: number) => {
        ...
        const existingSessionIndex = state.writingSessions.findIndex(
          s => s.date === today
        );
        ...
        if (existingSessionIndex >= 0) {
          ...
          updatedSessions[existingSessionIndex] = {
            ...updatedSessions[existingSessionIndex],
            wordCount:
              updatedSessions[existingSessionIndex].wordCount + wordCount,
          };
